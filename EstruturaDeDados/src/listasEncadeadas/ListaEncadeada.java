...
public class ListaEncadeada {
	...
	// Lista 2 Q2 Heraldo Correção questão 1
	
			public void removeRepetidos() throws IllegalAccessException {
				No atual = this.inicio;
				//int pos = 0;
				int removidos = 0;
				ListaEncadeada removiveis = new ListaEncadeada();
				for(int i = 0; i<this.totalDeElementos-removidos;i++) {
					No atual2 = atual.getProximo();
					for(int j=i+1; j<this.totalDeElementos;j++) {
				...
				for(int i = 0 ; i < removiveis.totalDeElementos; i++){
					//this.removeDoFinal((int) atualRemoviveis.getElemento() - i);
	...
	public void trocaValores(Object valor1, Object valor2) {
		No atual = this.inicio;
		int pos1 = -1, pos2 = -1;
		for(int i=0; i < this.totalDeElementos; i++) {
			if(atual.getElemento().equals(valor1) || atual.getElemento().equals(valor2)) {
				if(pos1>=0) {
					pos2 = i;
				}
			} else {
